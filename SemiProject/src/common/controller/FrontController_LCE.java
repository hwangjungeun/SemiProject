...
@WebServlet(
		...
		urlPatterns = { "*.go" }, 
		initParams = { 
				@WebInitParam(name = "propertyConfig", value = "C:/Users/sangs/git/SemiProject/SemiProject/WebContent/WEB-INF/Command.properties", description = "*.go 에 대한 클래스의 매핑파일")
		})
public class FrontController_LCE extends HttpServlet {
	...
	*/
		...
		try {
	        ...
	        */
			...
			while(en.hasMoreElements()) {
				...
				String key = (String) en.nextElement(); // Object타입을 String으로 casting
			...
			//	System.out.println("### 확인용 value => " + pr.getProperty(key));
				...
				*/
				...
				if(className != null) {
					className = className.trim(); // Command.properties에서 키의 맨 끝에 공백을 잘못 줬을수도있으니까, 안전하게 .trim()으로 공백제거
				...
				//	System.out.println("~~~ 확인용 obj.toString() => " + obj);
					/*
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 웹브라우저의 주소 입력창에서
		// http://localhost:9090/SemiProject/member/idDuplicateCheck.go?userid=leess 와 같이 입력되었더라면
		
	//	String url = request.getRequestURL().toString();
	//	System.out.println("~~~ 확인용 url => " + url);
		// ~~~ 확인용 url => http://localhost:9090/SemiProject/member/idDuplicateCheck.go
	...
	//	System.out.println("~~~ 확인용 uri => " + uri);
		// ~~~ 확인용 uri => /SemiProject/member/idDuplicateCheck.go
		
	//	System.out.println("~~~ 확인용 request.getContextPath() => " + request.getContextPath());
		// ~~~ 확인용 request.getContextPath() => /SemiProject
		
		String key = uri.substring(request.getContextPath().length()); // request.getContextPath().length()가 현재는 6이니까, 인덱스6부터 끝까지 substring
	//	System.out.println("~~~ 확인용 key => " + key);
		// ~~~ 확인용 key => /member/idDuplicateCheck.go
		
		// ~~~ 확인용 key => /index.go
		// ~~~ 확인용 key => /main.go
		
		AbstractController_LCE action = (AbstractController_LCE)cmdMap.get(key); // 부모클래스인 AbstractController로 형변환
		
		if(action == null) {
			System.out.println(">>> "+key+" 은 URI 패턴에 매핑된 클래스가 없습니다. <<<");
			// >>> /maadsfdsdn.up 은 URI 패턴에 매핑된 클래스가 없습니다. <<<
		}
		else {
			try {
				/*
